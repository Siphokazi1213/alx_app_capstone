  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-white rounded-3xl shadow-2xl w-full max-w-lg mx-auto transform transition-transform duration-500 ease-in-out hover:scale-[1.02]">
      ...
      <div className="w-full">
        ...
        <div className="mb-8">
          <label className="block text-sm font-medium text-gray-700 mb-2">Choose Your Difficulty</label>
          <select
        ...
        <button
          onClick={handleStart}
          // The button is disabled if either category or difficulty is not selected
...
const QuizPage = ({ questions, currentQuestionIndex, onAnswer, onGoBack, error, isLoading }) => {
  ...
  return (
    <div className="flex flex-col items-center p-8 bg-white rounded-3xl shadow-2xl w-full max-w-lg mx-auto">
      <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center" dangerouslySetInnerHTML={{ __html: currentQuestion.question }}></h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
        {currentQuestion.allAnswers.map((answer, index) => (
          <button
            ...
            className={`py-4 px-6 rounded-xl border-2 font-medium transition-all duration-200 ease-in-out transform hover:scale-105 ${getButtonColor(answer)}`}
...
const App = () => {
  ...
  // Function to start the quiz based on the selected category and difficulty
  const startQuiz = async (categoryId, difficulty) => {
    ...
    try {
      ...
      if (data.response_code !== 0) {
        setError("Could not retrieve questions. Try a different category or dificulty.");
        return;
      }
